...
class TelegramNotifier:
    ...
    def send_sell_ranking(self, results, top_n=5, max_score=-0.4, title="🔻 OPORTUNIDADES DE QUEDA IMINENTE"):
        """
        Envia ranking das moedas com maior potencial de queda (score mais negativo).
        """
        sell_list = [
            r for r in results
        ...
        # Ordenar do mais negativo para o menos negativo
        ...
        if running:
            ...
            if now >= next_scan_time:
                try:
                    ...
                    if results:
                        # Envio automático de compras (pode desabilitar se quiser)
...
async def sellranking_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    success = notifier.send_sell_ranking(
        last_scan_results,
        top_n=config.TOP_BUY_COUNT,
        max_score=-config.MIN_SCORE_FOR_RANKING,  # usa o mesmo limiar, mas negativo
...
def main():
    ...
    # Registra handlers (incluindo novos para venda)
    ...
    logger.info("Bot iniciado com suporte a comandos (compra e venda). Pressione Ctrl+C para parar.")
    app.run_polling()
