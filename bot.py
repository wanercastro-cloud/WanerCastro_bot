...
# Carrega .env apenas para testes locais (não usado no Actions)
load_dotenv()
...
# ========== LÓGICA DO SINAL (COM FILTRO) ==========
def check_signal(coin_id, df):
    """
    Verifica se as condições de alerta foram atendidas.
    Retorna dicionário com detalhes ou None.
    """
    ...
    ema28 = calculate_ema(close, 28)
    rsi = calculate_rsi(close, config.RSI_PERIOD)
    _, _, macd_hist = calculate_macd(close)

    # Filtro anti-falso sinal: ignora se tendência estiver muito forte (EMA7 > EMA14 > EMA28)
    tendencia_forte = (ema7.iloc[-1] > ema14.iloc[-1]) and (ema14.iloc[-1] > ema28.iloc[-1])
    if config.IGNORE_STRONG_TREND and tendencia_forte:
        logger.debug(f"{coin_id}: Tendência explosiva – ignorado.")
        return None

    # Condições de alerta (queda iminente)
