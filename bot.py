...
def run_overnight() -> None:
    ...
    best = [
        x
        for x in ranked
        if x.label in {"🌙 OVERNIGHT PREMIUM", "📈 CONTINUAÇÃO VÁLIDA"}
    ][: SETTINGS.overnight_top_n]
    ...
    if SETTINGS.adapt_weights:
        ...
        if updated:
            send(
                ...
                f"trend={updated['trend']:.2f}, "
                f"macd={updated['macd']:.2f}, "
                f"adx={updated['adx']:.2f}, "
                f"rsi={updated['rsi']:.2f}"
            )
