...
class TelegramNotifier:
    def __init__(self, token, chat_id):
        ...
        self.risk_freeze = {}  # {coin_symbol: last_alert_time}
    ...
    def is_frozen(self, symbol):
        """Verifica se um ativo está congelado devido a risco de repetição."""
        ...
        if datetime.now() - last_time < timedelta(hours=config.RISK_FREEZE_EXHAUSTION):
            return True
        # Descongela após o período
        del self.risk_freeze[symbol]
        return False

    def freeze(self, symbol):
        """Congela um ativo para evitar novos alertas."""
        self.risk_freeze[symbol] = datetime.now()

    def send_buy_ranking(self, results, top_n=5, min_score=0.6, title="🚀 TOP OPORTUNIDADES DE COMPRA"):
        """
        ...
        """
        # Filtrar compras fortes e não congeladas
        buy_list = [
            r for r in results
        ...
        # Congelar os ativos que entraram no ranking
        for item in top_buy:
...
def should_send_overnight():
    """Verifica se é hora de enviar o overnight pick (dentro de 1 minuto)."""
...
def main():
    ...
    if config.SEND_STARTUP_MESSAGE:
        notifier.send_message("🤖 Bot iniciado. Monitorando oportunidades de compra...")

    logger.info("Bot iniciado. Pressione Ctrl+C para parar.")
    overnight_sent_today = False  # Evita enviar mais de uma vez por dia
    ...
    while True:
        try:
            ...
            # 3. Enviar ranking normal (se houver)
            notifier.send_buy_ranking(
                results,
                top_n=config.TOP_BUY_COUNT,
                min_score=config.MIN_SCORE_FOR_RANKING,
                title="🚀 TOP OPORTUNIDADES DE COMPRA"
            )

            # 4. Enviar overnight picks se for a hora e ainda não enviou hoje
            if should_send_overnight() and not overnight_sent_today:
                ...
                notifier.send_buy_ranking(
                    ...
                    min_score=config.MIN_SCORE_FOR_RANKING,
                    title="🌙 PICKS OVERNIGHT (segure até amanhã)"
                )
