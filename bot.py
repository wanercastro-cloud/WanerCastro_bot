...
from scoring import score_coin, build_ranking_text, build_smartmoney_text

logging.basicConfig(
    format="%(asctime)s | %(levelname)s | %(name)s | %(message)s",
    level=logging.INFO,
)
log = logging.getLogger("ranking_bot")
    ...
    while True:
        try:
            ...
            if hhmm == OVERNIGHT_TIME and sent_date != now.date():
                ...
                sent_date = now.date()
                log.info("Ranking overnight enviado para o chat.")
        except Exception as e:
...
def main() -> None:
    ...
    print("Todas as variaveis obrigatorias carregadas")
