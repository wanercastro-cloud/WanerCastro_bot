...
def compute_rsi(series, period=14):
    """Calcula RSI a partir de uma série de preços de fechamento."""
    delta = series.diff()
...
def build_indicator_pack_from_market_chart(data):
    """
    Processa o JSON retornado pelo endpoint /coins/{id}/market_chart
    e retorna um dicionário com os indicadores finais.

    Parâmetros:
        data (dict): com chaves 'prices' e 'total_volumes'

    Retorna:
        dict ou None: se dados suficientes, retorna:
            {
                'rsi': float,
                'rsi_score': int,
                'macd_score': int,
    ...
    # Opcional: adicionar volumes (não usado nos indicadores, mas pode ser útil)
    volumes = data.get('total_volumes', [])
    if volumes:
        vol_df = pd.DataFrame(volumes, columns=['timestamp', 'volume'])
        vol_df['timestamp'] = pd.to_datetime(vol_df['timestamp'], unit='ms')
        vol_df.set_index('timestamp', inplace=True)
        # Usa concat com o índice já alinhado
        df = pd.concat([df, vol_df], axis=1).dropna()

    # Garantir que temos dados suficientes (pelo menos 50 velas)
    if len(df) < 50:
        return None
