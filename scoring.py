...
def overnight_score(coin, ind):
    ...
    # Tendência
    trend = 1.0 if (price > ema20 > ema50) else (0.55 if price > ema20 and ema20 >= ema50 else 0.10)
    ...
    # ATR eficiente, sem bagunça demais
    ...
    # Bollinger: não queremos colado no topo
    if bb_pos < 0.78:
        bb_score = 1.0
    elif bb_pos < 0.88:
        bb_score = 0.65
    ...
    # ROC: impulso saudável, sem histeria
    if 0.5 < roc <= 6:
        roc_score = 1.0
    elif 0 < roc <= 10:
        roc_score = 0.65
    ...
    # Penalidades de exaustão / pump recente
    ...
    exhaustion = min(penalty * 1.6, 100.0)
...
def build_overnight_ranking_text(results, top_n=3):
    picks = [r for r in results if r["grade"] in ("A+", "A")]
    if not picks:
        picks = [r for r in results if r["grade"] == "B"]
    if not picks:
        picks = results[:]
