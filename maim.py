...
LAST_SNAPSHOT: Dict[str, List[Dict[str, Any]]] = {"up": [], "down": []}
...
def build_block(title: str, rows: List[Dict[str, Any]]) -> str:
    if not rows:
    ...
    for i, r in enumerate(rows, start=1):
        ...
        lines.append(
            ...
            f"${r['price']:.6f} | mcap ${money(r['mcap'])} | vol ${money(r['vol24'])}"
        )
        lines.append("")
    return "\n".join(lines).strip()
...
async def send_telegram_raw(text: str) -> None:
...
async def scan_market() -> Dict[str, List[Dict[str, Any]]]:
    rows = await get_market()
    prepared = prepare_rows(rows)
    ranked = score_rows(prepared)
    LAST_SNAPSHOT["up"] = ranked["up"]
    LAST_SNAPSHOT["down"] = ranked["down"]
    return ranked
...
async def background_scanner(app: Application) -> None:
    while True:
        try:
            ranked = await scan_market()

            up_rows = ranked["up"][:3]
            down_rows = ranked["down"][:3]

            for r in up_rows:
                if can_send_alert("UP", r["symbol"]):
                    await send_telegram_raw(build_alert("UP", r))

            for r in down_rows:
                if can_send_alert("DOWN", r["symbol"]):
                    await send_telegram_raw(build_alert("DOWN", r))

        except Exception as e:
            msg = f"⚠️ Erro no scanner: {str(e)[:300]}"
            print(msg)
            try:
                await send_telegram_raw(msg)
            except Exception:
                pass
...
async def cmd_top(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ranked = await scan_market()
    msg = (
        build_block("📊 TOP SUBIDA", ranked["up"][:5]) +
        "\n\n" +
        build_block("📊 TOP DESCIDA", ranked["down"][:5])
    )
...
async def cmd_up(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ranked = await scan_market()
    msg = build_block("🚀 TOP SUBIDA", ranked["up"])
    await update.message.reply_text(msg)


async def cmd_down(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ranked = await scan_market()
    msg = build_block("📉 TOP DESCIDA", ranked["down"])
    await update.message.reply_text(msg)
...
async def post_init(app: Application) -> None:
    if SEND_STARTUP_MESSAGE:
        try:
            await send_telegram_raw("✅ Bot iniciado no Railway com comandos /ping /top /up /down")
        except Exception:
            pass
...
def validate_env() -> None:
    ...
    if not TG_BOT_TOKEN:
        raise RuntimeError("Defina TG_BOT_TOKEN no Railway.")
