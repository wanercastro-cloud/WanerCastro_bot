...
INTERVAL_SECONDS = int(os.getenv(“INTERVAL_SECONDS”, “14400”))  # padrão: 4h

def run_once():
if not CG_API_KEY:
print(”[WARN] CG_API_KEY não definida — API pública sem auth.”, file=sys.stderr)
...
def main():
print(f”[INFO] Iniciando — intervalo: {INTERVAL_SECONDS}s ({INTERVAL_SECONDS//3600}h)”, file=sys.stderr)
while True:
try:
run_once()
except Exception as e:
print(f”[ERR] Erro no ciclo: {e}”, file=sys.stderr)
print(f”[INFO] Próxima execução em {INTERVAL_SECONDS}s…”, file=sys.stderr)
